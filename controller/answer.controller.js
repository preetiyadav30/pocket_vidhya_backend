...
const answer_attempt = async (req, res, next) => {
    ...
    try {
        ...
        db.query(`select * from user where user_id='${decoded_Username}'`,(err1,res1)=>{
            if(err1){
                res.status(400).send({
                    success: false,
                    err: err1
                ...
                db.query(`Select * from questionnaire where question_id=? and Status="ACTIVE" and Language=?`, [req.body.question_id,res1[0].Language], (question_err, question_result) => {
                    if (question_err) {
                        res.status(400).send({
                            success: false,
                            err: question_err
                                ...
                                if (answerInsert_err) {
                                    ...
                                    db.query(`select *, count(distinct(question_id)) as total_Question_Answered from  answer where user_id=? and category=?`, [decoded_Username, question_result[0].category], (totalAnswer_err, totalAnswer_result, feilds) => {
                                        ...
                                        if (totalAnswer_result) {
                                            ...
                                            //This seelects the total no of Active questions from questionnaire table 
                                            // console.log(totalAnswer_result)
                                            db.query(`select question_id,count(question_id) as Total_Available_Questions from questionnaire where status='ACTIVE' and category='${question_result[0].category}' and Language='${res1[0].Language}'`, (totalQuestion_err, totalQuestion_result) => {
                                                if (totalQuestion_err) {
                                                    res.status(400).send({
                                                        success: false,
                                                        err: totalAnswer_err
                                                ...
                                                if (totalQuestion_result) {
                                                    ...
                                                    // console.log(totalQuestion_err)
                                                    db.query(`select count(distinct(question_id)) as total_skipd_questions from answer where isCorrect="null" and user_id=? and category=? and answer=""`, [decoded_Username, question_result[0].category], (totalQuestion_err, totalskipQuestion_result) => {
                                                        if (totalQuestion_err) {
                                                            res.status(400).send({
                                                                success: false,
                                                                err: totalAnswer_err
                                                        ...
                                                        if (totalskipQuestion_result) {
                                                            db.query(`select count(distinct(question_id)) as Total_Correct_Answers from  answer where user_id =? and isCorrect='1' and category=?`, [decoded_Username, question_result[0].category], (correctAnswer_err, correctAnswer_result) => {
        
                                                                if (correctAnswer_err) {
                                                                    res.status(400).send({
                                                                        success: false,
                                                                        err: correctAnswer_err
                                                                    })
                                                                }
                                                                if (correctAnswer_result) {
                                                                    // db
                                                                    ...
                                                                    const total_skipd_questions = totalskipQuestion_result[0].total_skipd_questions
        
                                                                    const Total_Questions_Attempted = totalAnswer_result[0].total_Question_Answered
        
                                                                    const Questions_RemainnigTo_Attempt = Total_Available_Questions - Total_Questions_Attempted
        
                                                                    const Total_Incorrect_Answers = Total_Questions_Attempted - (Total_Correct_Answers + total_skipd_questions)
                                                                
                                                                    if (Total_Questions_Attempted >= Total_Available_Questions) {
        
                                                                        db.query("Insert into attempts(user_id,category,q_Skipped,q_attempted,correct_Answers,Score,Language) values(?,?,?,?,?,?,?) ", [decoded_Username, question_result[0].category, total_skipd_questions, Total_Questions_Attempted, Total_Correct_Answers, Total_Correct_Answers,question_result[0].Language], (insertAttempt_err, insertAttempt_result) => {
                                                                            if (insertAttempt_err) {
                                                                                res.status(400).send({
                                                                                    success: false,
                                                                                    err: insertAttempt_err
                                                                            ...
                                                                            if (insertAttempt_result) {
                                                                                db.query('delete from answer where user_id=? and category=?', [decoded_Username, question_result[0].category], (vacantAnswer_err, vacantAnswer_result) => {
                                                                                    ...
                                                                                    if (vacantAnswer_result) {
                                                                                        ...
                                                                                        res.status(200).send({
                                                                                            Language:question_result[0].Language,
                                                                                            success: true,
                                                                                            isCorrect: isCorrectans,
                                                                                            msg: "Attempt Completed and counted",
                                                                                            Total_No_of_Questions: Total_Available_Questions,
