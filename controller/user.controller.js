...
const admin_update_question = async (req, res, next) => {
    ...
    try {
        ...
        await db.query(`Update questionnaire set Question=?,category=?,option1=?,option2=?,option3=?,option4=?,correct_Option=?,Description=?,Language=?,Status=?,updated_at=${now()},added_by where question_id=?`, [req.body.Question, req.body.category, req.body.option1, req.body.option2, req.body.option3, req.body.option4, req.body.correct_Option, req.body.Description, req.body.Language, req.body.Status,decoded_Username,req.params.question_id], (err, result, feilds) => {
            if (err) {
                res.status(401).send({
                    success: false,
                    err: err.message
